...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["cortex security audit", "Run a local security audit."],
          ["cortex security audit --deep", "Include best-effort live Gateway probe checks."],
          ["cortex security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["cortex security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Cortex security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("cortex security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("cortex security audit --fix")}`));
        if (!fixResult) {
