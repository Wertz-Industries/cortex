...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("cortex node install"), formatCliCommand("cortex node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("cortex node install --force")}`);
    }
    return;
  }
