...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Cortex and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["cortex update", "Update a source checkout (git)"],
        ["cortex update --channel beta", "Switch to beta channel (git + npm)"],
        ["cortex update --channel dev", "Switch to dev channel (git + npm)"],
        ["cortex update --tag beta", "One-off update to a dist-tag or version"],
        ["cortex update --dry-run", "Preview actions without changing anything"],
        ["cortex update --no-restart", "Update without restarting the service"],
        ["cortex update --json", "Output result as JSON"],
        ["cortex update --yes", "Non-interactive (accept downgrade prompts)"],
        ["cortex update wizard", "Interactive update wizard"],
        ["cortex --update", "Shorthand for cortex update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run cortex update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["cortex update status", "Show channel + version status."],
          ["cortex update status --json", "JSON output."],
          ["cortex update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
